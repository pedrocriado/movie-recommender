...
def evaluate(val_dataloader, model, criterion, opt, epoch):
    bar = tqdm(enumerate(val_dataloader), total=len(val_dataloader)) # Progress bar
    ...
    for i, (encoded_user_id, encoded_movie_id, rating, user_id, movie_id) in bar:
        encoded_user_id = encoded_user_id.to(opt.device)
        encoded_movie_id = encoded_movie_id.to(opt.device)
        rating = rating.to(opt.device)
        
        pred_rating = model(encoded_user_id, encoded_movie_id)
...
def train(train_dataloader, model, criterion, optimizer, opt, epoch, val_dataloader=None, wandb_log=True):
    ...
    bar = tqdm(enumerate(data_iterator), total=len(data_iterator)) # Progress bar

    # Iterate over the data and train the model
    for i , (encoded_user_id, encoded_movie_id, rating, user_id, book_id) in bar:
        ...
        if i % val_cycle == 0:
            bar.set_description("Epoch {} Loss: {:.4f}".format(epoch, running_loss / dataset_size))
            if wandb_log:
...
if __name__ == '__main__':
    ...
    for epoch in range(1):
        train(train_dataloader, model, criterion, optimizer, opt, epoch, val_dataloader=val_dataloader, wandb_log=False)
